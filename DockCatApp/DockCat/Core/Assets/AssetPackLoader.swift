...
final class AssetPackLoader {
    private static let defaultPackID = "bozi"
    ...
    private func syncDefaultPackBackup(to customRoot: URL) {
        ...
        guard let source = defaultPackCandidates().first(where: { isDirectory($0) }) else {
            DockCatLog.assets.error("Failed to seed bozi because bundled DefaultCat was not found")
            return
        }
        do {
            ...
            DockCatLog.assets.error("Failed to refresh bozi backup: \(error.localizedDescription)")
        }
    }
    ...
    private func seedDefaultPackIconsIfNeeded(in customRoot: URL) {
        ...
        do {
            ...
            DockCatLog.assets.error("Failed to seed bozi app icons: \(error.localizedDescription)")
        }
    ...
    private func repairDefaultPackManifestIconsIfNeeded(at root: URL) {
        ...
        guard fileManager.fileExists(atPath: manifestURL.path),
              ...
              isDefaultBoziManifest(manifest),
              manifest.appIcons == nil
        else {
            return
        }
        do {
            try defaultBoziManifestData(withAppIcons: true).write(to: manifestURL, options: .atomic)
        } catch {
            DockCatLog.assets.error("Failed to repair bozi manifest app icons: \(error.localizedDescription)")
        }
    }
    ...
    private func isDefaultBoziManifest(_ manifest: AssetManifest) -> Bool {
        manifest.id == "bozi"
            && manifest.name == "波子"
            && manifest.author == "genzuuuu"
    ...
    private func defaultBoziManifestData(withAppIcons: Bool) throws -> Data {
